...
# The rule all defines the targeted output-file of the pipeline
rule all:
    input:
        "05-results/PREP_Nextflow_EAGER_noReads.tsv"

# The rule samtools_sort_by_name sorts the bam-file of the mapped reads by name
rule samtools_sort_by_name:
    output:
        temp("tmp/eager_extract_unmapped/{lib}.nsorted.bam")
...
# The rule samtools_fixmate
rule samtools_fixmate:
    input:
        "tmp/eager_extract_unmapped/{lib}.nsorted.bam"
    output:
        temp("tmp/eager_extract_unmapped/{lib}.fixmate.bam")
